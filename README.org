...
No hidden property drawers!  The source Org syntax is also just five lines:
...
* Deprecation notice
This package works perfectly.  I don't expect it to need any patches, for a long time.

However, I *won't be extending this package*.  I'm heading a new direction per my epiphany on https://edstrom.dev/posts/2023-09-22.

If there's a bug, file an issue and I'm happy to help.
...
- [ ] Define an expression for a traditional front-back note-type instead of cloze-deletion.  One idea is a =#+begin_anki...#+end_anki= structure block that takes a bunch of parameters, but the extra 2 lines feel clunky in my head and I don't think I would use it much.  It's the perfect tech if you want fine control, but I'm going the other direction with some ideas to let us avoid the extra 2 lines.
  - [ ] Create a front-back note expression: a table of exactly two cells, be that 1x2 or 2x1.
    - [ ] Extend this so that if you have many rows, you can flag the table as a table of flashcards where each row is one flashcard.  Very compact!
- [ ] Create a fifth cloze note expression: rows in a single-column Org table, where each row expresses a paragraph in Anki.  But only if it's 3 cells or more.
...
- [ ] Include the file path as an extra field, so that the user can click the path inside Anki.
  - [ ] Instruct the user on how to make Anki always open links with emacsclient.
- [ ] For myself, I'd like a programmable way to include any extra field, since I would use that to include the URL to my web-published version of the same source file.  Then I could visit that URL while reviewing on the phone.
