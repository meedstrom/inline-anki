...
- The format is constrained -- you can't express every kind of flashcard possible in Anki.
  - For me this is a good trade, if that means they stick.  Flashcards never stayed long in my life due to clumsy systems.   Better simple flashcards than no flashcards.

------

Be aware: while this borrows code from anki-editor, it REMOVES some features you might expect:

- It uses only one deck ("Default")
- It uses only one note type ("Cloze"), with only one field ("Text")
- You cannot set tags per card, but the cards will inherit tags from the local Org subtree
