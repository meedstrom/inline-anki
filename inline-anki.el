...
(defcustom inline-anki-must-heed-gitignore t
  "If you don't know what gitignore is, you can disable this.
For those who do, this is mainly useful if you are gitignoring
duplicate files such as backups generated by Logseq - which would
result in duplicate Anki cards!

Requires git installed and available."
  :type 'boolean)
...
(defun inline-anki-push-notes-in-directory ()
  ...
  (interactive)
  (require 'asyncloop)
  (if (and inline-anki-must-heed-gitignore
           (inline-anki-git-repo-p ".")
           (not (executable-find "git")))
      (message "No git, can't push until you set `inline-anki-must-heed-gitignore' to nil")
        ...
        (list
         (lambda (_)
           (setq inline-anki--file-list
                 (if (and
                      (or inline-anki-must-heed-gitignore
                          (executable-find "git")))
