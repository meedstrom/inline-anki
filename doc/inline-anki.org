...
- =inline-anki-fields=
  - Default =("Text" "Extra")=.  A refactor likely coming soon.
...
By default, =inline-anki-fields= includes a field with the filename.  With addons such as https://ankiweb.net/shared/info/879473266, it should be possible to open links using emacsclient.  Instructions TBD.
...
- You cannot set tags on a per-card basis
  - The cards will still inherit tags from the local Org context (i.e. subtree and file-level tags)
    - Note that Anki tags lose much of their organizational purpose if all your notes live in Org files anyway -- you'll be organizing from within [[https://github.com/org-roam/org-roam][org-roam]] or whichever system you have, not from within Anki.  However, at least two use-cases remain:
      ...
      - You can use the tags to set up Custom Study sessions on a filtered subset of notes
        - Useful since the package only targets one deck
