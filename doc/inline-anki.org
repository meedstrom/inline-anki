...
This manual is for Inline Anki version 0.4.0.
...
- You cannot set tags on a per-note basis
  - The note will still inherit tags from the Org subtree
    - Note that Anki tags lose much of their organizational purpose if your notes primarily live in Org files anyway -- you'll be organizing them from within [[https://github.com/meedstrom/org-node][org-node]], org-roam or whichever system you have for browsing Org files, not from within Anki.  However, at least two use-cases remain:
      - You can display the tags inside cards as a contextual hint, [[https://edstrom.dev/grwhs/show-tags-as-helpful-hint-in-anki-cards][if you code this in the card template]]
...
- To identify a flashcard, inline-anki looks for one of the following things:
  ...
  - A magic string at the *end of a line*: =@anki= OR =^{anki}=.
    - In this case, you can omit the @ glyph.
  ...
  - A drawer named =:anki:=
    - When Anki assigns it an ID, it's renamed to something like =:anki-1693535436702:=.
