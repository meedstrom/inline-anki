...
#+BEGIN_QUOTE
Copyright (C) 2023 Martin Edström <meedstrom@teknik.io>
...
* Roadmap
- [ ] Allow hard-wrapping
- [X] Fix newline issues in multiline cards
...
- [ ] Define some expressions for a traditional front-back note-type instead of cloze-deletion.
  ...
  - [ ] A single-column or single-row 2-cell table.
...
- [X] Make so that commenting-out a flashcard expression also suspends it from review in Anki
- [X] Include the file-path as an extra field, so that the user can click the path inside Anki.
...
- Can I continue using my [org-drill/org-anki/...] cards?
  - Yes!  They don't conflict because those packages treat whole Org subtrees as their "data objects", and this package does not.  You can even have an inline-anki card /inside/ an org-anki subtree... not that you'd want to.
